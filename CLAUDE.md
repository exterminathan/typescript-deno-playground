...
|---|---|---|
| `main` | CalTrans Data Map (current). To start a new project branch, fork from commit `3dcd7e5` ("Removed Spotify, pre-fork commit") which is the clean pre-fork base. | [src/main.ts](src/main.ts) |
| `caltrans-tool` | Older variant of the CalTrans app. Slightly tweaked `index.html` / `.gitignore`. | [src/main.ts](src/main.ts) |
...
[.github/workflows/publish.yml](.github/workflows/publish.yml) — on push to `main`: `deno lint` → `deno check src/main.ts` → `deno task build` → upload `dist/` to GitHub Pages. Only `main` deploys; other branches do not auto-publish.
...
- [src/main.ts](src/main.ts) — Leaflet map, district buttons (1–12), data-type toggle buttons, geocoder, per-type popup renderers. Markers live in a single `L.layerGroup()` (`markerLayer`) and are only rebuilt when district/option toggles fire — no per-pan rebuild. Popups are bound lazily (`bindPopup(() => createPopup(data))`) so popup DOM is only created when actually opened. Fetched dictionaries are cached per `${district}-${binaryFlag}` for 5 minutes (`CACHE_TTL_MS`).
- [src/leafletWorkaround.ts](src/leafletWorkaround.ts) — fixes Leaflet's default marker icon URLs under Vite bundling. Required.
- [src/assets.d.ts](src/assets.d.ts) — declares `*.png` imports as default-exporting strings, so asset imports in `main.ts` don't need `// @ts-ignore`.
...
- `// deno-lint-ignore-file no-explicit-any` is used in `main.ts` because the CalTrans payloads are loosely typed at the boundary. Keep it scoped — don't broaden to a blanket ignore.
